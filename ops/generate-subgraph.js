...
async function generateSubgraph() {
  ...
  fs.writeFileSync(
    'subgraph.yaml', yaml.safeDump(subgraph, { noRefs: true }), 'utf-8'
  )
}
...
function combineFragments(fragments, isTemplate, addresses, missingAddresses) {
  return fragments.map(mapping => {
    ...
    var contractAddress
    if (isTemplate === false) {
      contractAddress = addresses[network][mapping.dao][mapping.contractName]
      ...
      if (!contractAddress) {
        ...
        const daoContract = `${mapping.contractName}-${mapping.dao}`;
