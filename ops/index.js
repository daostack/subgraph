...
async function configure({ env, ...rest }) {
	...
	fs.writeFileSync('config.json', JSON.stringify(config, undefined, 2), 'utf-8');
	...
	const schema = config =>
		subschemas.map(subschema => handlebars.compile(fs.readFileSync(subschema, 'utf-8'))(config)).join('\n\n');

	fs.writeFileSync('schema.graphql', schema(config), 'utf-8');

	const subgraph = handlebars.compile(fs.readFileSync('subgraph.handlebars.yaml', 'utf-8'));
