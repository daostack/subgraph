...
export function updateGSMCProposal(
  proposalId: Bytes,
  createdAt: BigInt,
  avatarAddress: Address,
  descriptionHash: string,
  ...
  let proposal = getProposal(proposalId.toHex());
  proposal.dao = avatarAddress.toHex();
  proposal.genericSchemeMultiCall = proposalId.toHex();
  proposal.createdAt = createdAt;
  proposal.descriptionHash = descriptionHash;
  proposal.scheme = crypto.keccak256(concat(avatarAddress, schemeAddress)).toHex();
  getProposalIPFSData(proposal);
...
export function updateProposalExecution(
  ...
  if (proposal.genericSchemeMultiCall !== null && (
    equalStrings(proposal.winningOutcome, 'Pass') && proposal.stage == 'Executed')
  ) {
