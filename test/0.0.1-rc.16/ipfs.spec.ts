...
describe('Domain Layer', () => {
  ...
  it('Sanity', async () => {
    ...
    proposalDescription = proposalIPFSDataLong.description;
    descHash = await writeProposalIPFS(proposalIPFSDataLong);
    const { proposalId: plong } = await propose({
      proposalDescHash: descHash,
      rep: 10,
      tokens: 10,
      eth: 10,
    ...
    proposal = (await sendQuery(getProposalLong)).proposal;
    expect(proposal).toMatchObject({
      id: plong,
      descriptionHash: descHash,
      title: proposalTitle,
