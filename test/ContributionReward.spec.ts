...
describe("ContributionReward", () => {
  ...
  beforeAll(async () => {
    ...
    contributionReward = new web3.eth.Contract(
      ContributionReward.abi,
      addresses.ContributionReward[0],
      opts
    );
  });
  ...
  it("Sanity", async () => {
    ...
    const nativeToken = await new web3.eth.Contract(
      DAOToken.abi,
      undefined,
      opts
    )
    ...
    const avatar = await new web3.eth.Contract(Avatar.abi, undefined, opts)
      .deploy({
        ...
        arguments: [
          "Test",
          nativeToken.options.address,
          reputation.options.address
        ]
      })
      .send();
    await externalToken.methods
      .transfer(avatar.options.address, "100000")
      .send();
    ...
    const absVote = await new web3.eth.Contract(
      AbsoluteVote.abi,
      undefined,
      opts
    )
    ...
    const ethTransferHelper = await new web3.eth.Contract(
      EthTransferHelper.abi,
      undefined,
      opts
    )
    ...
    const {
      ...
      blockNumber
    } = await absVote.methods
      .vote(proposalId, 1, accounts[0].address /* unused by the contract */)
    ...
    const {
      transactionHash: redeemReputationTxHash
    } = await contributionReward.methods
      .redeemReputation(proposalId, avatar.options.address)
      .send();
    ...
    const {
      transactionHash: redeemNativeTokenTxHash
    } = await contributionReward.methods
      .redeemNativeToken(proposalId, avatar.options.address)
      .send();
    ...
    const {
      transactionHash: redeemExternalTokenTxHash
    } = await contributionReward.methods
      .redeemExternalToken(proposalId, avatar.options.address)
      .send();
    ...
    await ethTransferHelper.methods
      .transfer(avatar.options.address)
      .send({ value: web3.utils.toWei("10", "ether") });
    //
    const {
      transactionHash: redeemEtherTxHash
    } = await contributionReward.methods
      .redeemEther(proposalId, avatar.options.address)
      .send({ gas: 1000000 });
